...
CREATE TABLE IF NOT EXISTS patients (
    ...
    birth_year          INTEGER,
    notes               TEXT,
    -- Mirrors db/migrations/005_ownership_fks.sql. Nullable forever:
    -- dev mode (no Authentik) and pre-migration rows both legitimately
    -- write NULL here. No FK declared because users only exists in PG
    -- (engine/users/schema.sql owns the SQLite users table on a separate
    -- file) — service code populates this with the Authentik-derived
    -- user id string.
...
CREATE TABLE IF NOT EXISTS treatments (
    ...
    notes               TEXT,
    created_by_user_id  TEXT,                 -- mirrors migration 005
    created_at          TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
);

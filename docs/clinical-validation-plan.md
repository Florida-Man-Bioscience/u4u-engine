...
| **Prospective observational (chosen design)** | Correlate the engine's genomic response predictions against observed per-peptide biomarker trajectories in real-world treatment-as-usual, across build/platform diversity | The defined clinical-validation study (§19.4) |
| **Decision-impact study** | Does the report change clinician decisions? | Utility, lower cost than outcomes RCT |
| **Randomized controlled trial** | Does using the tool *and the therapy* improve patient outcomes vs. control? | Efficacy/utility claims for the compounds themselves — an **excluded** path that stays outside product scope (§12.4, §19.5) |

**Chosen design.** The clinical-validation track is a **prospective, observational, multi-site cohort study** run in partnership with peptide-prescribing clinics. FMB enrolls consenting adults who **independently** have been prescribed peptide therapy by their own clinicians (treatment-as-usual), and performs longitudinal **observational tracking** of their progress using the per-peptide biomarker panels the engine proposes. The primary aim is to correlate the engine's genomic response predictions against observed per-peptide biomarker trajectories, establishing analytical and clinical validity of those predictions. **FMB does not prescribe, supply, dose, or direct any peptide** — it observes patients of partner clinics. Scope spans **all peptides partner clinics prescribe**, covering the full `PEPTIDE_MEASUREMENTS` panel (`engine/peptides/measurements.py`), including unapproved research peptides (observed, never supplied); GLP-1 receptor agonists are highlighted first for their grade-A evidence, but the design covers every peptide.
...
- Peptide response → the primary endpoints of the chosen observational study (§12.2) are the engine's **per-peptide biomarker panels** (source of truth: `PEPTIDE_MEASUREMENTS` in `engine/peptides/measurements.py`), tracked longitudinally against the engine's genomic response predictions. Eight of these markers carry citation-anchored A-D graded evidence in `data/biomarker_evidence.json` — the two grade-A entries are the class-qualified GLP-1 markers `Body weight (GLP-1 RA)` and `HbA1c (GLP-1 RA)`. These panels **must first be validated as response surrogates, not assumed**; alongside them, pre-specified established clinical endpoints (e.g., VAS pain, validated functional scores, imaging) anchor the reference standard.
...
The chosen observational design (§12.2) makes this boundary **cleaner, not weaker**: because FMB only observes patients who were independently prescribed peptide therapy by their own clinicians — never prescribing, supplying, dosing, or directing any compound — the study validates the **predictor** and the **biomarker trajectories as candidate response surrogates**, and structurally cannot and does not make an efficacy claim about any peptide therapy. Establishing that the engine predicts an observed biomarker trajectory says nothing about whether the therapy itself confers clinical benefit.

Implications for the validation program:
- **Validating the predictor does not validate the therapy.** Even a perfectly calibrated "responder" model predicts response to an intervention whose benefit/risk is itself unestablished. Confirming that observed biomarker trajectories match the engine's predictions validates the **predictor and its surrogate markers**, not the efficacy of the peptide.
...
Any evidence-generation involving human participants or their data (retrospective concordance on banked samples, the prospective observational clinic-partnership study, the peptide-response/predictor studies, biobank PRS validation) requires **IRB/ethics approval, informed consent appropriate to the use (including secondary use and return-of-results), and ICH-GCP-compliant conduct.** Using patient genomes to *study* an unvalidated predictor is research and must be governed as such — it cannot be done under the guise of clinical care.

The chosen clinical-validation study (§12.2, §19.4) is **prospective and observational**: FMB enrolls consenting adults already prescribed peptide therapy by their own clinicians (treatment-as-usual) and performs longitudinal observational tracking. Because it enrolls participants prospectively and handles clinic-held PHI, it is **not** exempt secondary-data research; it is **minimal-risk observational research eligible for expedited IRB review** (45 CFR 46.110, categories 5 and/or 4/7). It requires **informed consent plus HIPAA authorization** from each participant and a **BAA and/or DUA with each partner clinic** governing the PHI flow. Because FMB only observes — it never prescribes, supplies, doses, or directs any compound, including the unapproved research peptides, for which an adverse-event-observation posture applies — the study remains **outside IND** under 21 CFR 312.2(b). The **IRB of record is the UF single IRB (sIRB)** (settled 2026-06-30); partner sites cede review to it via reliance agreements (SMART IRB).

### 18.4 Informed consent and disclosure
Participant-facing consent must truthfully disclose: what is analyzed; what is and is not validated; that investigational-compound content (if retained at all) is not proof of benefit; data storage, sharing (including external services), retention, and rights; and the limitations (build/platform/ancestry/variant-class). GINA and anti-discrimination protections must be explained.

For the prospective observational clinic-partnership study, consent and disclosure additionally carry the **treatment-as-usual and observation-only** framing. The consent form plus **HIPAA authorization** must make plain that: participation is **observational tracking only** and FMB **does not prescribe, supply, dose, or direct** any peptide; the decision to start, continue, change, or stop peptide therapy rests solely with the participant and their own clinician; enrollment (or declining/withdrawing) has **no effect on the care the partner clinic provides**; and the study observes biomarker trajectories to test a research predictor whose validity is not yet established. Consent must be **free of coercion or undue influence** — no payment or benefit is contingent on continued therapy, and partner-clinic staff must not present enrollment as a condition of, or an inducement within, clinical care (§18.5). Adverse events observed during tracking are recorded and reported per the protocol even though FMB does not supply the compound.

### 18.5 Conflicts of interest
FMB **does not prescribe, supply, or sell** peptide therapies — it observes patients whom partner clinics have independently prescribed — so the classic "recommends and sells the same product" conflict does not apply to the study. The conflict of interest that **does** apply is FMB's commercial and scientific interest in seeing its **own predictor validated** while enrolling the **patients of partner clinics**. That must be governed transparently on two fronts:
- **No inducement of prescribing.** FMB's enrollment, observation, or partnership arrangements must not influence whether or how a partner clinic prescribes peptide therapy. Compensation to partner clinics (if any) must be fair-market-value for research activities only, never volume- or prescription-linked, so that observation never becomes an incentive to prescribe.
- **No coercion of participants.** Enrollment must be free of undue influence; declining or withdrawing must carry no penalty to the participant's clinical care (§18.4).

Recommendation and prediction logic must remain defensible on clinical grounds independent of commercial interest, and this independence — together with the enrollment/observation firewall between FMB and partner-clinic prescribing — is something the Advisory Board, the UF sIRB, and any auditor will scrutinize.
...
- **Run the defined clinical-validity study: the prospective, observational, multi-site clinic-partnership cohort (§12.2).** FMB enrolls consenting adults already prescribed peptide therapy by their own clinicians (treatment-as-usual) and performs longitudinal observational tracking, correlating the engine's genomic response predictions against observed per-peptide biomarker trajectories (endpoints per §12.3). Prerequisites in this phase: **UF sIRB** approval under **expedited review** (45 CFR 46.110 cat 5 and/or 4/7), **informed consent + HIPAA authorization**, and executed **BAA/DUA with each partner clinic** (§18.3-§18.4); the study stays **outside IND** (21 CFR 312.2(b)) because FMB only observes. As feasible, add decision-impact/utility studies (§12).
- Validate the per-peptide biomarker panels as **response surrogates** before relying on them as endpoints (§12.3), keeping the **peptide-evidence boundary** — this validates the predictor and its surrogates, not therapy efficacy (§12.4).
**Exit gate:** Medical-Director-accountable benefit-risk determination per reportable claim.

### 19.5 Phase 4 — Investigational-compound efficacy claims (multi-year, separate research track — excluded from product)
**Goal:** the predictor / receptor / tracking validation now lives in the Phase 3 observational study (§19.4); what remains here is the **therapy-efficacy** question, which stays **out of clinical scope** unless genuine human evidence for the compounds themselves is generated under formally controlled research.
- A clinical **efficacy claim for a peptide therapy** would require **adequate and well-controlled trials of the compound itself** — a separate, IND-governed (21 CFR 312.2(b) no longer applies once a compound is administered/directed for investigational purposes), interventional program that FMB's observational study deliberately does **not** undertake (§12.4).
- Until and unless such trials exist, **investigational-compound efficacy claims remain excluded from the product** (§12.4); the observational study validates the predictor and biomarker surrogates only, and makes no efficacy claim about any peptide.
- Note this track depends on world-state clinical evidence the software — and an observation-only study — cannot generate alone; timelines are inherently long and uncertain.

...
- **If both UF and the company are "engaged"** (UF investigator + company sponsor), expect a **single-IRB (sIRB) reliance agreement**. Note UF already operates a commercial-IRB reliance pathway — **IRB-04 / WCG (Western IRB)** — for studies ceded to WCG, which is the natural vehicle for a UF + sponsor arrangement.
...
- [ ] **GCP training** — required for NIH-funded clinical trials (note: the UF module `UF_CTS904_OLT` does **not** satisfy the NIH GCP requirement — use a compliant course).
...
- [ ] **Confirm IRB of record** (§2 decision) — IRB-01 vs. WCG/IRB-04 reliance.

...
*Cross-references: `docs/clinical-validation-plan.md` §3 (intended use / regulatory posture), §9–§11 (analytical & clinical validation designs = the study endpoints), §13 (privacy/security = the data-management plan), §18 (governance/ethics rationale); `docs/enrollment-strategy.md` (Lane C recruitment / site network / N≈1000 ops target); `docs/validation/prediction-efficacy-audit/` (PEAP readiness before locked-primary enrollment).*
